...
public abstract class DefaultEM {
  // ==== stop criterion
  private int maxNumIter = 100;// run at most 100 iterations (i.e., number of funcion and gradient
                               // evaluation) for this particular run
  private double relativeLikelihoodThreshold = 1e-5;// if the relative change of the function value
                                                    // is smaller than this value, then we terminate
  private int maxConvergeNum = 1000;// if the number of times that the likelihood does not change,
                                    // then stop
  ...
  public void runEM() {
    ...
    while (numCalls == 0 || (isEMConverged() == false) && (numCalls <= maxNumIter)) {
      ...
      if (tLikelihood < lastLikelihood) {
        System.out.println("EM returns a bad optimal value; best: " + tLikelihood + "; last: "
            + lastLikelihood);
        System.exit(1);
      }
      ...
      if (numCalls != 0
          && Math.abs(lastLikelihood - tLikelihood) / lastLikelihood < relativeLikelihoodThreshold) {
        checkConverge++;
        if (checkConverge >= maxConvergeNum) {// does not change for several consecutive times
          System.out
              .println("EM early stops because the likelihood does not change for several iterationss; break at iter "
                  + numCalls);
          break;
        }
      } else {
