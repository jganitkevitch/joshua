...
public class NbestExpbleuGradientComputer extends GradientComputer {
  ...
  public NbestExpbleuGradientComputer(String nbestFile, String[] refFiles, int numFeatures,
      double gainFactor, double scalingFactor, double temperature,
      boolean shouldComputeGradientForScalingFactor, int numSentence) {
  ...
  public void reComputeFunctionValueAndGradient(double[] theta) {
    ...
    this.minLen = closestLen; // use closest length, instead of the minimum length to compute the
                              // length penalty;
  ...
  private void finalizeFunAndGradients() {
    ...
    if (x > 0) {
      y =
          ((1 - N * x) * myexp(-N * x) + myexp(-2 * N * x)) / (myexp(-N * x) + 1)
              / (myexp(-N * x) + 1);
    } else {
    ...
    for (int i = 0; i < this.numFeatures; ++i) {
      ...
      for (int j = 0; j < 4; ++j) {
        this.gradientsForTheta[i] -=
            1.0 / 4.0 / (ngramMatches[0] - j * this.numSentence)
                * ngramMatchesGradients.get(0).get(i);
      }
      double dx =
          this.minLen / this.ngramMatches[0] / this.ngramMatches[0]
              * this.ngramMatchesGradients.get(0).get(i);
