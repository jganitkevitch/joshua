...
public abstract class SplitHg {
  HashMap<HGNode, ArrayList<VirtualItem>> g_tbl_split_virtual_items =
      new HashMap<HGNode, ArrayList<VirtualItem>>();// Key: item; Value: a list of split virtual
                                                    // items
  ...
  protected abstract void process_one_combination_axiom(HGNode parent_item,
      HashMap virtual_item_sigs, HyperEdge cur_dt);

  protected abstract void process_one_combination_nonaxiom(HGNode parent_item,
      HashMap virtual_item_sigs, HyperEdge cur_dt, ArrayList<VirtualItem> l_ant_virtual_item);
  ...
  private static HyperEdge clone_deduction(VirtualDeduction virtual_dt) {
    ...
    if (originalEdge.getAntNodes() != null)
      l_ant_items = new ArrayList<HGNode>(originalEdge.getAntNodes());// l_ant_items will be changed
                                                                      // in get_1best_tree_item
    HyperEdge res =
        new HyperEdge(originalEdge.getRule(), originalEdge.bestDerivationLogP,
            originalEdge.getTransitionLogP(false), l_ant_items, null);
    return res;
  }
  ...
  private void redo_combine(HyperEdge cur_dt, HashMap virtual_item_sigs, HGNode parent_item) {
    ...
    if (l_ant_items != null) {
      if (l_ant_items.size() == 1) {// arity: one
        ...
        ArrayList<VirtualItem> l_virtual_items =
            (ArrayList<VirtualItem>) g_tbl_split_virtual_items.get(it);
        ...
        ArrayList<VirtualItem> l_virtual_items1 =
            (ArrayList<VirtualItem>) g_tbl_split_virtual_items.get(it1);
        ArrayList<VirtualItem> l_virtual_items2 =
            (ArrayList<VirtualItem>) g_tbl_split_virtual_items.get(it2);
  ...
  protected void add_deduction(HGNode parent_item, HashMap virtual_item_sigs,
      VirtualDeduction t_ded, DPState dpstate, boolean maintain_onebest_only) {
    if (t_ded == null) {
      System.out.println("deduction is null");
      System.exit(0);
    }
