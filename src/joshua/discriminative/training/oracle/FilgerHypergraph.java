...
/**
 * Given a hypergraph and a string, generate a new hypergraph that contains only derivations that
 * lead to the string
 */
public class FilgerHypergraph {
  ...
  /**
   * for each unique substring in the sentence, associate it with a list of possible spans in the
   * sentence
   */
  ...
  private boolean operationNode(HGNode it) {
    ...
    if (onNode) onNodesTbl.put(it, true);
  ...
  private boolean operationHyperedge(HyperEdge dt) {
    ...
    if (matchTargetString(dt) == false) return false;
  ...
  /**
   * assuming all the ant nodes are on, now we see if the application of the rule matches a larger
   * span
   * */
