...
/**
 * This programm assumes the transition_cost in the input hypergraph is properly
 * set!!!!!!!!!!!!!!!!!
 * */
...
public class StringSumInHG {
  ...
  public StringSumInHG(KBestExtractor kbestextractor, ApproximateFilterHGByOneString filter_) {
    kbestExtractor = kbestextractor;
    p_filter = filter_;
  }
  ...
  public double computeSumForString(HyperGraph hg, int sentenceID, String ref_string) {
    ...
    int[] ref_sent_wrds_in = Vocabulary.addAll(ref_string);
    ...
    // debug
    /*
     * double inside_outside_scaling_factor =1.0; TrivialInsideOutside p_inside_outside = new
     * TrivialInsideOutside(); p_inside_outside.run_inside_outside(filted_hg, 0, 1,
     * inside_outside_scaling_factor);//ADD_MODE=0=sum; LOG_SEMIRING=1; double io_norm_constant =
     * p_inside_outside.get_normalization_constant(); p_inside_outside.clear_state();
     * 
     * p_inside_outside.run_inside_outside(hg, 0, 1,
     * inside_outside_scaling_factor);//ADD_MODE=0=sum; LOG_SEMIRING=1;
    ...
    kbestExtractor.lazyKBestExtractOnHG(filtedHG, null, this.topN, sentenceID,
        nonUniqueNbestStrings);// ???????????????????????????
    ...
    for (String derivation_string : nonUniqueNbestStrings) {
      ...
      double log_prob = new Double(fds[fds.length - 1]);// TODO: use inside_outside_scaling_factor
                                                        // here
    ...
    System.out.println("good_sum: " + good_log_sum_prob + "; good_num: " + num_good_derivations
        + "; bad_sum: " + bad_log_sum_prob + "; bad_num: " + num_bad_derivations);
  ...
  public static void main(String[] args) throws InterruptedException, IOException {
    ...
    KBestExtractor kbest_extractor = new KBestExtractor(true, false, false, false, false, true);// ????????????
    ApproximateFilterHGByOneString filter =
        new ApproximateFilterHGByOneString(baseline_lm_feat_id, baseline_lm_order);
    StringSumInHG p_sumer = new StringSumInHG(kbest_extractor, filter);
    ...
    for (int sent_id = 0; sent_id < num_sents; sent_id++) {
      ...
      if (skip == false) {
        for (String unique_string : nonUniqueNbestStrings) {
          ...
          String hyp_string = fds[1];
          String[] wrds = hyp_string.split("\\s+");
          if (wrds.length > max_num_words) break;
