...
public class RuleExtraction {
  ...
  public static void main(String[] args) {
    ...
    BufferedWriter t_writer_rules =
        FileUtilityOld.getWriteFileStream(
            "C:\\data_disk\\java_work_space\\SyntaxMT\\phraseExtraction\\extract.rule.tbl", "UTF8");
  ...
  /*
   * public static Hashtable setup_align_tbl(String align_str){ Hashtable res_tbl = new Hashtable();
   * String[] links = align_str.split("\\s+"); for(int i=0; i < links.length; i++){
   * 
   * String[] ids = links[i].split("-"); Vector t_aligns =null;
   * if(res_tbl.containsKey(ids[0])){//already have entry t_aligns = (Vector) res_tbl.get(ids[0]);
   * }else{ t_aligns = new Vector(); res_tbl.put(ids[0], t_aligns); //System.out.println(ids[0]); }
   * t_aligns.add(ids[1]); } //debug for(int i=0; i<9 ; i++){
   * if(res_tbl.containsKey(Integer.toString(i))){ System.out.println(i + ": " +
   * res_tbl.get(Integer.toString(i)).toString()); } }
   * 
   * return res_tbl; }
   */
