...
public class TreeNode {
  ...
  Vector span = null;// according to ISI's syntaxMT, this only consider the first/last word index in
                     // the english string, e.g., 1-3 or 10-10
    ...
    if (res_match == 0) {// first, see whether the name match
      ...
      if (from.children.size() == 1) {// In "OR" condition, must have one and only one child
        for (int i = 0; i < this.children.size(); i++) {
          /*
           * note that we should not clear the tag_tbl if one of the chilren fails, that's why we
           * delete tag_tbl.clear()
           */
          if ((this.children.get(i)).isSubsume(from.children.get(0), tag_tbl) == true) {// any
                                                                                        // sucess
            t_res = true;
            break;
          }
  ...
  public boolean setFrontierFlag() {
    if (span == null) {// unaligned source node
      isFrontier = false;
      return false;
    } else if (complementSpans == null || complementSpans.size() <= 0) {// span over all the english
                                                                        // words
      isFrontier = true;
      return true;
    } else {
  ...
  public void deriveRule(Hashtable rule_tbl, int len_tgt, BufferedWriter out) {
    ...
    if (isFrontier == true && children.size() > 1) {// only extract rule for frontier node with more
                                                    // than one children
      int[] x_id = new int[1];
      x_id[0] = 0;

      String[] v_rhs_frags = new String[len_tgt];// the index is the start pos in the target, value
                                                 // is the rhs symbol
      String[] v_rhs_unaligned = new String[len_tgt + 1];// the index remember how many spans should
                                                         // put before it
   ...
   * need to do full-chilren match 2: "OR" name-matches-stage susscessful, means that for children
   * match, we should return true as long as one match the pattern, do not consider the number of
   * chilren
   */
  private int patternMatch(String from, String to) {
    if (from.matches("\\|.+")) {
      String from2 = from.replaceFirst("\\|", "");
      if (to.compareTo(from2) == 0) return 2;
    } else {
      if (from.compareTo("*") == 0 || to.compareTo("*") == 0)// anything will match
        return 1;
      else if (from.matches("\\!.+")) {
        String from2 = from.replaceFirst("\\!", "");
        if (to.compareTo(from2) != 0) return 1;
      } else if (to.matches("\\!.+")) {
        String to2 = to.replaceFirst("\\!", "");
        if (from.compareTo(to2) != 0) return 1;
  ...
  private void ctrlDeriveSubrule(int[] x_id, String[] str_lhs, String[] v_rhs_frags,
      String[] v_rhs_unaligned) {
  ...
  private void deriveSubrule(int[] x_id, String[] str_lhs, String[] v_rhs_frags,
      String[] v_rhs_unaligned) {
    if (this.isFrontier == true) {// note: pre-terminal can be frontier node
      ...
      if (span == null) {
        ...
        addRhs(v_rhs_unaligned, v_rhs_frags.length, terminalSymbol);// remember how many continuos
                                                                    // span before me
      } else {// non-frontier pre-terminal
        /*
         * TODO: now, we sort the words according to their start span, this may not be good for
         * non-coninous translation for example, a prominent rile <=> ����(prominent) ����(a role),
         * may need to a rule "(NP (ADJP (JJ (���� f))) (NP (NN (���� f)))) => ���� ����" problem is
         * due to 1-to-m non-continuous translation
         */
