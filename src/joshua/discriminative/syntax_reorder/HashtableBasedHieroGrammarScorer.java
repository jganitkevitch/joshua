...
public class HashtableBasedHieroGrammarScorer {
  public static int NUM_FEATS = 4;// P(e|f),P(f|e), P_lex(e|f) , P_lex(f|e)
  HashMap p_gram = new HashMap();

  public HashtableBasedHieroGrammarScorer(int num_feats) {
    NUM_FEATS = num_feats;
  }

  public void addRawRule(Rule rl) {
  ...
  public void score_grammar() {
    ...
    while (rules.hasNext()) {
      ...
      // System.out.println("f_sum " + fr_sum + " e_sum " + eng_sum + " old: "+rl.feat_scores +
      // " new " + new_scores);
      rl.print_info();
    }
    System.out.println("invalid is " + num_infinite + "; number of unique rules are "
        + p_gram.size());
    // dump the rules??
