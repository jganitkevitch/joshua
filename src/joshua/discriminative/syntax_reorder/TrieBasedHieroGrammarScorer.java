...
/*
 * public interfaces TMGrammar: init and load the grammar TrieNode: match symbol for next layer
 * RuleBin: get sorted rules Rule: rule information
 */
...
public class TrieBasedHieroGrammarScorer {
  ...
  // this method should be called such that all the rules in rulebin are sorted, this will avoid
  // synchronization for get_sorted_rules function
  private void ensure_grammar_sorted() {
    if (root != null) root.ensure_sorted();
  }

  protected void print_grammar() {
    System.out.println("###########Grammar###########");
    System.out.println(String.format(
        "####num_rules: %d; num_bins: %d; num_pruned: %d; sumest_cost: %.5f", num_rule_read,
        num_rule_bin, num_rule_pruned, tem_estcost));
    /*
  ...
  public class TrieNode {
    ...
    public TrieNode match_symbol(int sym_id) {// looking for the next layer trinode corresponding to
                                              // this symbol
      if (tbl_children == null) return null;
    ...
    private void ensure_sorted() {
      if (rule_bin != null) rule_bin.get_sorted_rules();
  ...
  public class RuleBin {
    ...
    // this method need to be synchronized as we will call this function only after the decoding
    // begins
    ...
    private void add_rule(Rule rl) {
      if (heap_rules == null) heap_rules = new PriorityQueue(1, Rule.FrequencyComparator);// TODO:
                                                                                          // initial
                                                                                          // capacity?
  ...
  public static class Rule {
    ...
    // public int arity=0;//TODO: disk-grammar does not have this information, so, arity-penalty
    // feature is not supported in disk-grammar
    ...
    public void print_info() {
      // Support.write_log("Rule is: "+ lhs + " ||| " + Support.arrayToString(french, " ") + " ||| "
      // + Support.arrayToString(english, " ") + " |||", level);
      System.out.print("Rule is: " + Vocabulary.word(lhs) + " ||| ");
      for (int i = 0; i < french.length; i++)
        System.out.print(Vocabulary.word(french[i]) + " ");
      System.out.print("||| ");
      for (int i = 0; i < english.length; i++)
        System.out.print(Vocabulary.word(english[i]) + " ");
      System.out.print("||| ");
