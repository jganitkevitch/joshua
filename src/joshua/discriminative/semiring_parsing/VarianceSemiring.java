...
/**
 * This class implements an element in a ConvolutionSemiring This class does not depend on the
 * structure, e.g., hypergraph, lattice, or nbest
 * */

/**
 * we should not normalize the probability at each intermediate node because our model is a global
 * model???
 */
...
public class VarianceSemiring implements CompositeSemiring {
  ...
  public VarianceSemiring(double logProb_, SignedValue factor1_, SignedValue factor2_,
      SignedValue combinedfactor_) {
    logProb = logProb_;
    factor1 = factor1_;
    factor2 = factor2_;
  ...
  public void multi(CompositeSemiring b, AtomicSemiring atomic) {
    ...
    // combinedfacotr = a.prob * b.combinedfacotr + a.combinedfacotr * b.prob + a.factor1 *
    // b.factor2 + a.factor2 * b.factor1
    // this.combinedfactor = Math.exp(oldLogProb)* b2.combinedfactor + Math.exp(b2.logProb) *
    // oldCombineFactor + oldFactor1*b2.factor2 + oldFactor2* b2.factor1;
    SignedValue part1 =
        SignedValue.add(SignedValue.multi(this.logProb, b2.combinedfactor),
            SignedValue.multi(b2.logProb, this.combinedfactor));
    SignedValue part2 =
        SignedValue.add(SignedValue.multi(this.factor1, b2.factor2),
            SignedValue.multi(this.factor2, b2.factor1));
    this.combinedfactor = SignedValue.add(part1, part2);
    ...
    this.factor2 =
        SignedValue.add(SignedValue.multi(this.logProb, b2.factor2),
  ...
  public void printInfor() {
    System.out.println("prob: " + logProb + "; factor1: " + factor1.convertRealValue()
        + "; factor2: " + factor2.convertRealValue() + "; combined:"
        + combinedfactor.convertRealValue());
  }
  ...
  public void printInfor2() {
    ...
    System.out.println("prob: " + logProb + "; factor1: " + factor1 + "; factor2: " + factor2
        + "; combined:" + combinedfactor);
    System.out.println("true factor1: " + true_factor1 + "; true factor2: " + true_factor2
        + "; true combined:" + true_combinedfactor);
  }
