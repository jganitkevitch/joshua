 ...
 * compact structure, e.g., hypergraph, lattice, or nbest
 * */
...
public class ExpectationSemiring implements CompositeSemiring {
  private double logProb;// maybe un-normalized (this is fine for factor1, as we will divide do
                         // factor1/norm in normalizeFactors())
  ...
  public void add(CompositeSemiring b, AtomicSemiring atomic) {
    ...
    // this.factor1 = atomic.add_in_atomic_semiring(this.factor1,
    // ((ConvolutionSemiring2)b).factor1);
  ...
  public void multi(CompositeSemiring b, AtomicSemiring atomic) {
    ...
    // this.factor1 = Math.exp(oldLogProb)*b2.factor1 + Math.exp(b2.logProb)*oldFactor1;//real
    // semiring
  ...
  public void printInfor2() {
    ...
    System.out.println("unnormalize logProb: " + logProb + "; factor1: " + factor1
        + "; true_factor: " + true_factor.convertRealValue());
  }
