...
public class HypLenSquareExpectation extends DefaultSemiringParser {
  ...
  @Override
  protected VarianceSemiring getHyperedgeSemiringWeight(HyperEdge dt, HGNode parent_item,
    ...
    if (p_atomic_semiring.ATOMIC_SEMIRING == AtomicSemiring.LOG_SEMIRING) {
      ...
      SignedValue factor2 = SignedValue.multi(logProb, SignedValue.createSignedValue(val2));

      // double combined = Math.exp(logProb)*val*val2;//real semiring
      SignedValue combined = factor1;
      combined = SignedValue.multi(combined, SignedValue.createSignedValue(val2));
