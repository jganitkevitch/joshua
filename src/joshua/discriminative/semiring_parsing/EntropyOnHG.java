...
public class EntropyOnHG extends DefaultSemiringParser {
  ...
  protected ExpectationSemiring getHyperedgeSemiringWeight(HyperEdge dt, HGNode parent_item,
    ...
    if (p_atomic_semiring.ATOMIC_SEMIRING == AtomicSemiring.LOG_SEMIRING) {
      ...
      double val = scale * dt.getTransitionLogP(false);// s(x,y); to compute E(s(x,y)); s(x,y) is
                                                       // the linear combintation (considered
                                                       // scaling factor)

      // double factor1 = Math.exp(prob)*val; //real semiring
      SignedValue factor1 = SignedValue.multi(logProb, SignedValue.createSignedValue(val));
