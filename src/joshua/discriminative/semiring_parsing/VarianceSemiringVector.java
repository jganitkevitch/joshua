...
/**
 * while the logProb and factor1 are shared by all features, the second factor is a vector, so does
 * the combinedfactor
 */
...
public class VarianceSemiringVector implements CompositeSemiring {
  ...
  public VarianceSemiringVector(double logProb_, SignedValue factor1_, SignedValue[] factor2_,
  ...
  public void multi(CompositeSemiring b, AtomicSemiring atomic) {
    ...
    for (int i = 0; i < vectorSize; i++) {
      ...
      // this.combinedfactor[i] = Math.exp(oldLogProb)* b2.combinedfactor[i] + Math.exp(b2.logProb)
      // * oldCombinedfactor[i] + oldFactor1*b2.factor2[i] + oldFactor2[i]* b2.factor1;
      SignedValue part1 =
          SignedValue.add(SignedValue.multi(this.logProb, b2.combinedfactor[i]),
              SignedValue.multi(b2.logProb, this.combinedfactor[i]));
      SignedValue part2 =
          SignedValue.add(SignedValue.multi(this.factor1, b2.factor2[i]),
              SignedValue.multi(this.factor2[i], b2.factor1));
      this.combinedfactor[i] = SignedValue.add(part1, part2);


      // this.factor2[i] = Math.exp(oldLogProb)* b2.factor2[i] + Math.exp(b2.logProb) *
      // oldFactor2[i];
      this.factor2[i] =
          SignedValue.add(SignedValue.multi(this.logProb, b2.factor2[i]),
