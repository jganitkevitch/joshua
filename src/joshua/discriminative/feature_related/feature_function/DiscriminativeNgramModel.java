...
public class DiscriminativeNgramModel extends DefaultStatefulFF {
  ...
  public DiscriminativeNgramModel(int ngramStateID, int featID, int startNgramOrder,
      int endNgramOrder, String ngramModelFile, double weight, int baselineLMOrder) {
  ...
  public double estimateFutureLogP(Rule rule, DPState curDPState, int sentID) {
    ...
    return computeLogP(ngramExtractor.getFutureNgrams(rule, curDPState, startNgramOrder,
        endNgramOrder));
  }
  ...
  public double transitionLogP(Rule rule, List<HGNode> antNodes, int spanStart, int spanEnd,
    ...
    return computeLogP(ngramExtractor.getTransitionNgrams(rule, antNodes, startNgramOrder,
        endNgramOrder));
  }

  public double finalTransitionLogP(HGNode antNode, int spanStart, int spanEnd, SourcePath srcPath,
      int sentID) {
    return computeLogP(ngramExtractor.getFinalTransitionNgrams(antNode, startNgramOrder,
        endNgramOrder));
  }
