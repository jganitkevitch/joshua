...
public class NgramFT extends AbstractFeatureTemplate {
  ...
  public NgramFT(boolean useIntegerNgram, int ngramStateID, int baselineLMOrder, int startOrder,
      int endOrder) {
  ...
  public void getFeatureCounts(HyperEdge dt, HashMap<String, Double> featureTbl,
  ...
  public void getFeatureCounts(Rule rule, List<HGNode> antNodes,
    ...
    if (rule == null)
      ngramsTbl =
          ngramExtractor.getFinalTransitionNgrams(antNodes.get(0), startNgramOrder, endNgramOrder);
    else
      ngramsTbl =
  ...
  public void estimateFeatureCounts(Rule rule, HashMap<String, Double> featureTbl,
      HashSet<String> restrictedFeatureSet, double scale) {
    HashMap<String, Integer> ngramsTbl =
        ngramExtractor.getRuleNgrams(rule, startNgramOrder, endNgramOrder);
    if (ngramsTbl != null) {
      for (Map.Entry<String, Integer> entry : ngramsTbl.entrySet()) {
        String ngramFeatKey = entry.getKey();
        if (restrictedFeatureSet == null || restrictedFeatureSet.contains(ngramFeatKey) == true) {
