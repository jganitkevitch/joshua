...
public class FeatureIntersection {
  ...
  public static void main(String[] args) {
    ...
    if (args.length < 8) {
      System.out.println("wrong command, correct command should be: java Perceptron_HG "
          + "is_crf lf_train_items lf_train_rules lf_orc_items lf_orc_rules f_l_num_sents "
          + "f_data_sel f_model_out_prefix use_tm_feat use_lm_feat use_edge_bigram_feat_only "
          + "f_feature_set use_joint_tm_lm_feature");
    ...
    HashMap<Integer, Boolean> tbl_sent_selected = DiscriminativeSupport.setupDataSelTbl(f_data_sel);// for
                                                                                                    // data
                                                                                                    // selection
    ...
    for (int fid = 0; fid < l_file_train_items.size(); fid++) {
      ...
      for (int sent_id = 0; sent_id < total_num_sent; sent_id++) {
        ...
        if (hg != null) // sent is not skipped
          FeatureExtractionHG.featureExtractionOnHG(hg, featureIntersectionSet,
    ...
    FileUtilityOld.printHashTblAboveThreshold(featureIntersectionSet,
        featureFile + ".intersection", false, 0, false, false, null);
  }
}
