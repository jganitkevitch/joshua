...
public class CrossEntropyOnHG extends DefaultSemiringParser {
  ...
  public CrossEntropyOnHG(int semiring, int add_mode, double scale,
      List<FeatureFunction> pFeatFunctions, List<FeatureFunction> qFeatFunctions) {
  ...
  protected ExpectationSemiring getHyperedgeSemiringWeight(HyperEdge dt, HGNode parent_item,
    ...
    if (p_atomic_semiring.ATOMIC_SEMIRING == AtomicSemiring.LOG_SEMIRING) {
      ...
      double valQ = -scale * computeTransitionCost(parent_item, dt, qFeatFunctions);// from
                                                                                    // q;//s(x,y);
                                                                                    // to compute
                                                                                    // E(s(x,y));
                                                                                    // real semiring
      ...
      SignedValue factor1 = SignedValue.multi(logProbP, SignedValue.createSignedValue(valQ));
  ...
  static private double computeTransitionCost(HGNode parentNode, HyperEdge dt,
      List<FeatureFunction> featFunctions) {

    double[] transitionCosts =
        ComputeNodeResult.computeModelTransitionLogPs(featFunctions, dt, parentNode.i,
            parentNode.j, -1);
