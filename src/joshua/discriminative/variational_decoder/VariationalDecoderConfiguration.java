...
public class VariationalDecoderConfiguration {
  ...
  private static final Logger logger = Logger.getLogger(VariationalDecoderConfiguration.class
      .getName());
  ...
  public static void initializeModels(String configFile, List<FeatureFunction> featFunctions,
    ...
    while ((line = FileUtilityOld.readLineLzf(configReader)) != null) {
      ...
      if (line.indexOf("=") != -1) { // parameters
        ...
        if (fds[0].compareTo("vbaseline") == 0 && fds.length == 2) {
          ...
          FeatureFunction ff =
              new EdgeTblBasedBaselineFF(ngramStateID + 1 + featFunctions.size(), baselineWeight);
          ...
          // baseline combo features: vbaselinecombo list-of-baseline-features (each one should be
          // " pos_id||inter-weight ") weight
          ...
          FeatureFunction ff =
              new BaselineComboFF(ngramStateID + 1 + featFunctions.size(), weight, positions,
                  interWeights);
          featFunctions.add(ff);
          ...
          /*
           * double constituentWeight = new Double(fds[1].trim()); HashMap<HyperEdge,Double> tbl =
           * new HashMap<HyperEdge,Double>(); FeatureFunction ff = new
           * BaselineFF(baselineLMFeatID+1+featFunctions.size(), constituentWeight, tbl);
           * FeatureTemplate ft = new FeatureTemplateConstituent(); VariationalLMApproximator rmodel
           * = new VariationalLMApproximator(ff, ft, tbl); featFunctions.add(ff);
          ...
          if (vlmOrder > baselineLMOrder) {
            System.out
                .println("varatioanl_ngram_order is greater than baseline_lm_order; must be wrong");
            System.exit(1);
          }
          double weight = new Double(fds[2].trim());
          FeatureTemplate ft = new NgramFT(true, ngramStateID, baselineLMOrder, vlmOrder, vlmOrder);
          FeatureTemplateBasedFF ff =
              new FeatureTemplateBasedFF(ngramStateID + 1 + featFunctions.size(), weight, ft);

          VariationalNgramApproximator rmodel = new VariationalNgramApproximator(ft, 1.0, vlmOrder);
          featFunctions.add(ff);
