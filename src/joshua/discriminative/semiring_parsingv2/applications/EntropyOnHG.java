...
public class EntropyOnHG
    extends DefaultInsideSemiringParser<ExpectationSemiring<LogSemiring, ScalarPM>> {
  double scale;
  ...
  public static void main(String[] args) {
    ...
    int numSrcWords = 1;
    if (args.length >= 4) numSrcWords = new Integer(args[3].trim());
    ...
    DiskHyperGraph dhg_test = new DiskHyperGraph(ngramStateID, true, null); //
    ...
    for (int sent_id = 0; sent_id < num_sents; sent_id++) {
      ...
      ExpectationSemiring<LogSemiring, ScalarPM> goalSemiring =
          (ExpectationSemiring<LogSemiring, ScalarPM>) ds.getGoalK();
      ...
      double entropy =
          goalSemiring.getP().getLogValue() - goalSemiring.getR().getValue().convertToRealValue();
      System.out.println("entropy is " + entropy);
      sumEntropy += entropy;
    }
    System.out.println("scale=" + scale + "; num_sents=" + num_sents + "; numSrcWords="
        + numSrcWords);
