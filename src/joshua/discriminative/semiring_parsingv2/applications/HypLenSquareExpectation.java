...
public class HypLenSquareExpectation
    extends
    DefaultIOParserWithXLinearCombinator<ExpectationSemiring<LogSemiring, ScalarPM>, ExpectationSemiringPM<LogSemiring, ScalarPM, ScalarPM, ScalarPM, ScalarBO>> {

  private static final Logger logger = Logger.getLogger(HypLenSquareExpectation.class.getName());
  ...
  @Override
  protected ExpectationSemiringPM<LogSemiring, ScalarPM, ScalarPM, ScalarPM, ScalarBO> createNewXWeight() {
    ScalarPM s = new ScalarPM();
    ScalarPM t = new ScalarPM();
    return new ExpectationSemiringPM<LogSemiring, ScalarPM, ScalarPM, ScalarPM, ScalarBO>(s, t,
        pBilinearOperator);
  }
  ...
  @Override
  protected ExpectationSemiringPM<LogSemiring, ScalarPM, ScalarPM, ScalarPM, ScalarBO> getEdgeXWeight(
      HyperEdge dt, HGNode parent_item) {
    // == p
  ...
  @Override
  protected ExpectationSemiring<LogSemiring, ScalarPM> getEdgeKWeight(HyperEdge dt,
      HGNode parent_item) {
    // == p
    double logProb = scale * dt.getTransitionLogP(false);
    LogSemiring p = new LogSemiring(logProb);
  ...
  public void normalizeGoal() {
    ...
    ExpectationSemiringPM<LogSemiring, ScalarPM, ScalarPM, ScalarPM, ScalarBO> goalXVal =
        getGoalX();
  ...
  public static void main(String[] args) {
    if (args.length < 1) {
      System.out
          .println("Wrong command: java HypLenSquareExpectation  f_nbest_prefix scale numSent ....");
      System.exit(1);
    }
    ...
    double scale = 1;
    if (args.length >= 2) scale = new Double(args[1].trim());

    int num_sents = 5;
    if (args.length >= 3) num_sents = new Integer(args[2].trim());
    ...
    DiskHyperGraph diskHG = new DiskHyperGraph(baseline_lm_feat_id, true, null); // have model costs
                                                                                 // stored
