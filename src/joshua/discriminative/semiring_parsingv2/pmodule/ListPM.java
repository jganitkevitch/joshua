...
public class ListPM implements PModule<LogSemiring, ListPM> {
  ...
  /*
   * public void add(ListPM b) { for(Integer id : b.value.getIds()){ SignedValue valB =
   * b.value.getValueAt(id); SignedValue valA = this.value.getValueAt(id); if(valA!=null){
   * valA.add(valB); }else{ this.value.addInToArray(id, SignedValue.duplicate(valB) ); } } }
   */
