...
public class SparseMap implements SparseArray<SignedValue, SparseMap> {
   ...
   * 
   * HashMap<Integer, SignedValue> res = new HashMap<Integer, SignedValue>(); for(Integer id :
   * getIds()){ SignedValue val = getValueAt(id).duplicate(); res.put(id, val); } return new
   * SparseMap(res); }
   */
