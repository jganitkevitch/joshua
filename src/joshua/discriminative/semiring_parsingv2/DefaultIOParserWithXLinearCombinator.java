...
/**
 * This implements the algorithm of Figure-4 in the emnlp paper (Li and Eisner, 2009). To compute a
 * new second-order statistics, we just need to specifiy a weight for each hyperedge. Specifically,
 * we need to implement the following four functions 1. createNewKWeight 2. createNewXWeight 3.
 * getEdgeKWeight 4. getEdgeXWeight
 * */

/**
 * In addition to run the regular inside-outside algorithm, this also collects the posterior count
 * along with the outside parsing pass
 * */
...
public abstract class DefaultIOParserWithXLinearCombinator<K extends Semiring<K>, X extends PModule<K, X>>
  ...
  /**
   * since K and X are not concrete yet, only the extended class knows how to normalize
   */
  ...
  /**
   * this will run outside, and collect posterior counts
   */
  @Override
  final protected void outsideEstimationOverHyperedge(HyperEdge dt, HGNode parentNode,
