...
/**
 * This implements the algorithm of Figure-2 in the emnlp paper (Li and Eisner, 2009). To compute a
 * new first-order statistics, we just need to specifiy a weight for each hyperedge. Specifically,
 * we need to implement the following two functions 1. createNewSemiringMember 2.
 * getHyperedgeSemiringWeight
 * */


/**
 * semiring parsing on a Hypergraph, The semiring weight at each hyperedge is specificed by
 * getHyperedgeSemiringWeight()
 */
...
public abstract class DefaultInsideSemiringParser<K extends Semiring<K>> {
  ...
  protected K insideEstimationOverNode(HGNode it) {
    ...
    if (insideSemiringWeightsTable.containsKey(it)) return insideSemiringWeightsTable.get(it);
