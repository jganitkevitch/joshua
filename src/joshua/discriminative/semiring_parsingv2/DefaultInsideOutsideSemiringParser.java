...
/**
 * This implements the algorithm of Figure-3 in the emnlp paper (Li and Eisner, 2009), in addition
 * to the algorithm of Figure-2 that is implemented by the parent class DefaultInsideSemiringParser.
 * We may not want to extend this class directly. Instead, we will extend its child class
 * DefaultIOParserWithXLinearCombinator, which runs inside-outside and also collects the posterior
 * counts
 * */
...
public abstract class DefaultInsideOutsideSemiringParser<K extends Semiring<K>>
  ...
  /**
   * for each node, remember how many hyperedges pointering to me, this is needed for outside
   * estimation. An node will recursive call its incoming hyperedges to do outside-estimation only
   * after it itself is done with outside estimation, this is necessary because the outside
   * estimation of the node's incoding edges require the node's outside value.
   * 
   * TODO: A better approach is to first get topological order of the hypergraph and then run inside
   * in topological order, and outside in inverse-topological order.
   */
  ...
  /**
   * for correctness and saving memory, external class should call this method
   */
  @Override
  ...
  final protected void outsideEstimationOverNode(HGNode node, K parentNodeOutsideWeight,
      HyperEdge parentEdge, K parentEdgeWeight) {
    ...
    if (null == numCalled || 0 == numCalled) {
      ...
      throw new RuntimeException(
          "un-expected call (the number of calls is greater than the number of parent hyperedges), must be wrong");
    }
  ...
  protected void outsideEstimationOverHyperedge(HyperEdge dt, HGNode parentNode,
      K parentNodeOutsideWeight) {
  ...
  /**
   * This function will be used to get the number of parent hyperedges, which in turn will be used
   * for outside estimation
   */
